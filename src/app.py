...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///'+path+'//currency_exchangeRates_db.sqlite3'
...
def index():  
 ...
 if request.method == 'POST':
   ...
   if currency_exists is None: 
      ...
      currency_info = request.form['get_currency_info']
 ...
 currencyExchnage = currency_exchangeRates_db.query.filter_by(base_currency=currency).all()
 return render_template('index.html', selected_currency=currency, currencyexchange=currencyExchnage)


# Get the daily currency updates from USD to GBP,CAD,JPY,INR,HKD
